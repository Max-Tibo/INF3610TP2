...
int create_tasks() {
	...
	OSTaskCreate(TaskGeneratePacket, NULL, &TaskReceiveStk[TASK_STK_SIZE - 1],
	TASK_GENERATE_PRIO);

	OSTaskCreate(TaskVerifySource, NULL,
			&TaskVerifySourceStk[TASK_STK_SIZE - 1], TASK_STOP_PRIO);

	OSTaskCreate(TaskVerifyCRC, NULL, &TaskVerifyCRCStk[TASK_STK_SIZE - 1],
	TASK_RESET_PRIO);

	OSTaskCreate(TaskStats, NULL, &TaskStatsStk[TASK_STK_SIZE - 1],
	TASK_STATS_PRIO);

	OSTaskCreate(TaskComputing, NULL, &TaskComputeStk[TASK_STK_SIZE - 1],
	TASK_COMPUTING_PRIO);

	OSTaskCreate(TaskForwarding, NULL, &TaskForwardingStk[TASK_STK_SIZE - 1],
	TASK_FORWARDING_PRIO);

	OSTaskCreate(TaskPrint, &print_param[0], &TaskPrint1Stk[TASK_STK_SIZE - 1],
	TASK_PRINT1_PRIO);

	OSTaskCreate(TaskPrint, &print_param[1], &TaskPrint2Stk[TASK_STK_SIZE - 1],
	TASK_PRINT2_PRIO);

	OSTaskCreate(TaskPrint, &print_param[2], &TaskPrint3Stk[TASK_STK_SIZE - 1],
	TASK_PRINT3_PRIO);
...
int create_events() {
	...
	mutexStats = OSMutexCreate(MUT_STATS_PRIO, &err);
...
void TaskGeneratePacket(void *data) {
	...
	while (true) {
		...
		if (isGenPhase) {
			...
			err_msg("Error accepting mutex in TaskGeneratePacket", err);
			...
			if (shouldSlowThingsDown) {
				...
				xil_printf(
						"GENERATE : ********Génération du Paquet # %d ******** \n",
						nbPacketCrees);
...
void TaskVerifySource(void *data) {
	...
	while (true) {
		...
		err_msg("Pend mutexPacketSourceRejete in TaskVerifySource", err);
		...
		err_msg("Post mutexPacketSourceRejete in TaskVerifySource", err);
...
void TaskComputing(void *pdata) {
	...
	while (true) {
		...
		err_msg("inputQ in TaskComputing", err);
		...
		(packet->src >= REJECT_LOW4 && packet->src <= REJECT_HIGH4)) {
			...
			err_msg("Pend mutexPacketSourceRejete  in TaskComputing", err);
			...
			err_msg("Post mutexPacketSourceRejete in TaskComputing", err);
			...
			err_msg("Pend mutexPacketCRCRejete in TaskComputing", err);
			...
			err_msg("Post mutexPacketCRCRejete in TaskComputing", err);
			...
			if (err == OS_ERR_Q_FULL) {
				...
				err_msg("Pend mutexMemory in TaskComputing", err);

				free(packet);

				err = OSMutexPost(mutexMemory);

				err_msg("Post mutexMemory in TaskComputing", err);

				OSMutexPend(mutexPrinting, 0, &err);

				err_msg("Post mutexPrinting in TaskComputing", err);

				xil_printf(
						"Packet_Video rejete  in TaskComputing, queue full\n");
				...
				err_msg("highQ  in TaskComputing", err);
				...
				err_msg("Pend mutexPacketTraites  in TaskComputing", err);

				nbPacketTraites++;

				err = OSMutexPost(mutexPacketTraites);

				err_msg("Post mutexPacketTraites  in TaskComputing", err);
			...
			err = OSQQuery(mediumQ,&qInfo);
			max_msg_medium = qInfo.OSQSize;
			...
			err = OSQQuery(mediumQ,&qInfo);
			moyenne_msg_medium += qInfo.OSNMsgs;
			...
			if (err == OS_ERR_Q_FULL) {
				...
				xil_printf(
						"Packet audio rejete  in TaskComputing, queue full\n");
				...
				err_msg("mediumQ in TaskComputing", err);
			...
			if (err == OS_ERR_Q_FULL) {
				...
				xil_printf(
						"Packet autre rejete queue  in TaskComputing full\n");
				...
				err_msg("lowQ in TaskComputing", err);
			...
			xil_printf("WARNING: Unknown packet type  in TaskComputing!\n");
		...
		clk_end = OSTimeGet();
		clk = clk_end - clk_init;

		OSMutexPend(mutexPrinting, 0, &err);

		err_msg("Post mutexPrinting in TaskComputing", err);

		xil_printf("Time to compute a packet is : %d\n", clk);
...
void TaskForwarding(void *pdata) {
	...
	while (true) {
		...
		int attenteActive = 220000;


		packet = OSQAccept(highQ, &err);

		while(attenteActive>0)
			attenteActive--;

		attenteActive = 220000;

		//err_msg("Packet is not in highQ", err);

		if (packet == NULL) {

			packet = OSQAccept(mediumQ, &err);

			//err_msg("Packet is not in mediumQ", err);
		...
		if (packet == NULL) {
			...
			//err_msg("Packet is not in lowQ", err);
			...
			if (packet->dst >= INT1_LOW && packet->dst <= INT1_HIGH) {
				...
				//err_msg("Packet is not between INT1_LOW and INT1_HIGH  in TaskForwarding",err);
				...
				//err_msg("Packet is not between INT2_LOW and INT2_HIGH in TaskForwarding",err);
				...
				//err_msg("Packet is not between INT3_LOW and INT3_HIGH in TaskForwarding",err);
				...
				err_msg("Error accepting mutex in TaskForwarding", err);
				...
				*packet3 = *packet;
				...
				err_msg("Error posting mutex in TaskForwarding", err);

				err = OSMboxPost(mbox[0], packet);

				err_msg("Error posting mbox[0] in TaskForwarding", err);

				err = OSMboxPost(mbox[1], packet2);

				err_msg("Error posting mbox[1] in TaskForwarding", err);

				err = OSMboxPost(mbox[2], packet3);

				err_msg("Error posting mbox[2] in TaskForwarding", err);
...
void TaskStats(void *pdata) {
	...
	while (true) {
		...
		if(mode_profilage==0){
			...
			err_msg("Error accepting semaphore in TaskStats", err);

			OSMutexPend(mutexPrinting, 0, &err);

			err_msg("Error accepting mutex in TaskStats", err);
			...
			err_msg("Error posting mutex in TaskStats", err);

		/* À compléter */
		} else xil_printf("Pas dans le mode profilage");
	}
...
void TaskPrint(void *data) {
	...
	while (true) {
		...
		err_msg("OSMboxPend in TaskPrint", err);

		if (packet != NULL) {

			OSMutexPend(mutexPrinting, 0, &err);

			err_msg("mutexPrinting in TaskPrint", err);

			xil_printf(
					"INT %d - SRC %08x - DST %08x - TYPE %d - CRC  %d - DATA %x\n",
					intID, packet->src, packet->dst, packet->type, packet->crc,
					packet->data);
			...
			err_msg("mutexMemory in TaskPrint", err);
